const app = Vue.createApp({
  data() {
    return {
      timezone: Intl.DateTimeFormat().resolvedOptions().timeZone, // Dynamically determine timezone
    };
  },
});

app.component("authors-posts", {
  template: `
   <li 
        v-for="post in posts" 
        :key="post.id" 
      >
        <div class="author_container">
          <img 
            :src="getAuthor(post.author_id).avatar_url" 
            alt="Author avatar" 
            class="author"
          />
          <h4>{{ getAuthor(post.author_id).name }}</h4>
          <p>
